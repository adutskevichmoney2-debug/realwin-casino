...
Views.admin=function(){
 ...
 async function rUsers(){
  USERS_CACHE=await CLOUD.adminUsers();
  ...
  const draw=q=>{
   ...
   </tbody></table></div>`:`<div class="empty">${ic('search',28)}<div class="t">${t('adm.nodata')}</div></div>`;
   $$('.js-urow',box).forEach(r=>r.onclick=e=>{if(e.target.closest('button'))return;openPlayerCard(r.dataset.u)});
   $$('.js-open',box).forEach(b=>b.onclick=e=>{e.stopPropagation();openPlayerCard(b.dataset.u)});
  };
  $('.js-usearch',box).oninput=e=>draw(e.target.value);
  draw('');
 }
 async function openPlayerCard(uid2){
  const m=openModal(`<div class="m-body"><div class="empty">${ic('refresh',30)}</div></div>`,{klass:'xl'});
  const d=await CLOUD.adminUserCard(uid2);
  if(!d.p){m.close();toast('err',t('cl.err'));return}
  const p=d.p;
  const usd=d.wl.reduce((a,w)=>a+(+w.balance)*(RATES[w.coin]||0),0);
  const depS=d.tx.filter(x=>x.type==='dep').reduce((a,x)=>a+(+x.amount)*(RATES[x.coin]||1),0);
  const wdS=d.tx.filter(x=>x.type==='wd').reduce((a,x)=>a+Math.abs(+x.amount)*(RATES[x.coin]||1),0);
  const txRows=d.tx.map(x=>`<tr><td style="font-weight:700">${t('tx.'+x.type)}${x.meta?` <span class="dim" style="font-size:10.5px">\u00b7 ${esc(String(x.meta))}</span>`:''}</td>
   <td class="mono ${+x.amount>0?'up':''}">${+x.amount>0?'+':''}${fc(+x.amount,x.coin)} ${x.coin}</td>
   <td class="dim" style="text-align:right;font-size:11.5px;white-space:nowrap">${dts(Date.parse(x.created_at))}</td></tr>`).join('');
  const betRows=d.bets.map(b=>`<tr><td style="font-weight:700">${esc(b.kind==='sport'?((b.meta&&b.meta.label)||'Sports'):(gameBy(b.game)?gameBy(b.game).name:b.game||'\u2014'))} <span class="badge gray" style="font-size:9px">${b.kind}</span></td>
  ...
  $('.m-body',m.el).innerHTML=`
   <div style="display:flex;gap:14px;align-items:center;flex-wrap:wrap;padding-right:34px">
    <span class="avatar" style="width:54px;height:54px;border-radius:16px;font-size:20px;flex:none;background:linear-gradient(135deg,hsl(${p.avatar_hue||210} 70% 55%),hsl(${((p.avatar_hue||210)+40)%360} 70% 38%))">${esc(p.username[0].toUpperCase())}</span>
    ...
    <div style="display:flex;gap:8px;flex:none">
     ...
     ${p.role==='player'?`<button class="btn sm ${p.banned?'soft':'danger'} js-cban">${p.banned?t('adm.unban'):t('adm.ban')}</button>`:''}
    </div></div>
   <div class="pstats" style="margin:18px 0 14px">
    <div class="stat"><span class="si">${ic('wallet',18)}</span><span><span class="sv">${fusd(usd)}</span><br><span class="sl">${t('adm.balance')}</span></span></div>
    <div class="stat"><span class="si">${ic('chart',18)}</span><span><span class="sv">${fusd(+p.wagered)}</span><br><span class="sl">${t('prof.wagered')}</span></span></div>
    <div class="stat"><span class="si">${ic('arrDn',18)}</span><span><span class="sv">${fusd(depS)}</span><br><span class="sl">${t('adm.deposits')}</span></span></div>
    <div class="stat"><span class="si">${ic('arrUp',18)}</span><span><span class="sv">${fusd(wdS)}</span><br><span class="sl">${t('adm.withdrawals')}</span></span></div>
   </div>
   <div class="dd-h" style="padding:6px 2px">${t('adm.balances')}</div>
   <div style="display:flex;gap:8px;flex-wrap:wrap;margin:6px 0 16px">${d.wl.filter(w=>+w.balance>0).map(w=>`<span class="chip" style="cursor:default">${w.coin}: <b class="mono">\u00a0${fc(+w.balance,w.coin)}</b></span>`).join('')||`<span class="dim">${t('adm.nodata')}</span>`}</div>
   <div class="dd-h" style="padding:6px 2px">${t('adm.tx')} (${d.tx.length})</div>
   <div style="max-height:230px;overflow:auto;border:1px solid var(--line);border-radius:12px;margin:6px 0 16px">${d.tx.length?`<table class="tbl"><tbody>${txRows}</tbody></table>`:`<div class="empty" style="padding:18px">${t('adm.nodata')}</div>`}</div>
   <div class="dd-h" style="padding:6px 2px">${t('prof.bets')} (${d.bets.length})</div>
   <div style="max-height:230px;overflow:auto;border:1px solid var(--line);border-radius:12px;margin:6px 0 16px">${d.bets.length?`<table class="tbl"><tbody>${betRows}</tbody></table>`:`<div class="empty" style="padding:18px">${t('adm.nodata')}</div>`}</div>
   <div class="dd-h" style="padding:6px 2px">${t('adm.devices')} (${d.devices.length})</div>
  ...
  $$('[data-tk]',m.el).forEach(r=>r.onclick=()=>openTicketThread(+r.dataset.tk,'#'+r.dataset.tk+' \u00b7 '+r.dataset.s,true));
 }
